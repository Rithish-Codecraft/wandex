...
<head>
    ...
    <style type="text/css">
        ...
        #container {{ display: flex; flex-direction: row; height: 520px; width: 100%; border: 1px solid #334155; border-radius: 12px; overflow: hidden; }}
        #mynetwork {{ flex: 3; height: 100%; background-color: #0b0f19; position: relative; }}
        #sidebar {{ flex: 1; min-width: 270px; background-color: #1e293b; border-left: 1px solid #334155; padding: 1.25rem; overflow-y: auto; display: flex; flex-direction: column; gap: 1rem; }}
        #search-box {{ display: flex; gap: 6px; }}
        #search-input {{ flex: 1; background-color: #0f172a; border: 1px solid #475569; border-radius: 6px; color: #f8fafc; padding: 8px 10px; font-size: 0.85rem; outline: none; }}
        #search-input:focus {{ border-color: #6366f1; }}
        #btn-search, #btn-reset {{ background-color: #6366f1; border: none; border-radius: 6px; color: #ffffff; padding: 8px 14px; font-size: 0.85rem; font-weight: 600; cursor: pointer; transition: background-color 0.2s; }}
        #btn-search:hover, #btn-reset:hover {{ background-color: #4f46e5; }}
        #btn-reset {{ background-color: #475569; width: 100%; margin-top: auto; }}
        #btn-reset:hover {{ background-color: #334155; }}
        #legend {{ position: absolute; top: 15px; left: 15px; background: rgba(15, 23, 42, 0.85); border: 1px solid #334155; border-radius: 8px; padding: 10px; z-index: 10; font-size: 0.75rem; display: flex; flex-direction: column; gap: 6px; backdrop-filter: blur(4px); }}
        .legend-item {{ display: flex; align-items: center; gap: 8px; }}
        .legend-color {{ width: 12px; height: 12px; border-radius: 50%; display: inline-block; }}
        h3, h4 {{ margin: 0; }}
        .default-text {{ color: #94a3b8; font-style: italic; font-size: 0.85rem; text-align: center; margin: auto 0; }}
    </style>
</head>
<body>
<div id="container">
...
<script type="text/javascript">
    ...
    var options = {{
        nodes: {{ shape: 'dot', size: 20, shadow: true, font: {{ size: 14, face: 'Helvetica', color: '#f8fafc' }}, borderWidth: 2, borderWidthSelected: 4 }},
        edges: {{ width: 2.5, shadow: true, font: {{ size: 11, color: '#94a3b8', align: 'top' }}, arrows: {{ to: {{enabled: true, scaleFactor: 0.6}} }} }},
        physics: {{ barnesHut: {{ gravitationalConstant: -2000, centralGravity: 0.2, springLength: 120, springConstant: 0.04, damping: 0.09, avoidOverlap: 0.35 }}, maxVelocity: 45, minVelocity: 0.1, solver: 'barnesHut', stabilization: {{ enabled: true, iterations: 600, updateInterval: 100, fit: true }} }}
    }};
    var network = new vis.Network(container, data, options);
    const nodeMap = {{}};
    nodesArray.forEach(n => {{ nodeMap[n.id] = n; }});
    const adjList = {{}};
    nodesArray.forEach(n => {{ adjList[n.id] = {{ incoming: [], outgoing: [] }}; }});
    edgesArray.forEach(e => {{
        if (adjList[e.from]) {{ adjList[e.from].outgoing.push({{ to: e.to, rel: e.label }}); }}
        if (adjList[e.to]) {{ adjList[e.to].incoming.push({{ from: e.from, rel: e.label }}); }}
    }});
    network.on("selectNode", function (params) {{
        var selectedNodeId = params.nodes[0];
        if (selectedNodeId.toString().startsWith("desc_")) {{ return; }}
    ...
    function showDefaultSidebar() {{
        document.getElementById("detail-content").innerHTML = `<p class="default-text">Click on any node in the graph to view details and expand its contents.</p>`;
    }}
    function selectNodeById(nodeId) {{
        ...
        network.focus(nodeId, {{ scale: 1.1, animation: {{ duration: 500, easingFunction: "easeInOutQuad" }} }});
    }}
    function toggleNodeExpansion(nodeId) {{
        ...
        if (nodes.get(descNodeId)) {{
            ...
            nodes.add({{
                ...
                label: descText,
                shape: 'box',
                color: {{ background: '#1e293b', border: parentNode ? parentNode.color : '#6366f1', highlight: {{ background: '#0f172a', border: parentNode ? parentNode.color : '#6366f1' }} }},
    ...
    function collapseAllDescriptions() {{
        ...
        var toRemove = [];
        allNodes.forEach(n => {{ if (n.id && n.id.toString().startsWith("desc_")) {{ toRemove.push(n.id); }} }});
        if (toRemove.length > 0) {{ nodes.remove(toRemove); }}
    }}
    function showNodeDetails(nodeId) {{
        const node = nodeMap[nodeId];
        if (!node) return;
        let html = `<div style="display:flex; justify-content:space-between; align-items:center;"><span style="background-color: ${{node.color}}; color:#ffffff; padding:4px 8px; border-radius:12px; font-size:0.75rem; font-weight:bold; text-transform:uppercase;">${{node.title ? node.title.replace('Type: ', '') : 'Concept'}}</span></div><h3 style="margin-top:0.25rem; margin-bottom:0.5rem; color:#f8fafc; font-size:1.25rem; font-weight:700;">${{node.label}}</h3><p style="font-size:0.9rem; color:#cbd5e1; line-height:1.4; background:rgba(255,255,255,0.02); padding:8px 12px; border-radius:6px; border-left:3px solid ${{node.color}}; margin-top:0.5rem; margin-bottom:1rem;">${{node.description}}</p>`;
        const outgoing = adjList[nodeId].outgoing;
        if (outgoing.length > 0) {{ html += `<div style="margin-bottom:1rem;"><h4 style="margin-top:0; margin-bottom:0.5rem; color:#94a3b8; font-size:0.8rem; text-transform:uppercase; letter-spacing:0.05em;">Connected Concepts (Outgoing)</h4><div style="display:flex; flex-direction:column; gap:6px;">`; outgoing.forEach(edge => {{ const dest = nodeMap[edge.to]; if (dest) {{ html += `<div style="background-color:rgba(255,255,255,0.03); border:1px solid #334155; padding:6px 10px; border-radius:6px; font-size:0.85rem; cursor:pointer;" onclick="selectNodeById('${{edge.to}}')"><span style="color:#94a3b8;">— ${{edge.rel}} →</span> <strong style="color:${{dest.color}};">${{dest.label}}</strong></div>`; }} }}); html += `</div></div>`; }}
        const incoming = adjList[nodeId].incoming;
        if (incoming.length > 0) {{ html += `<div><h4 style="margin-top:0; margin-bottom:0.5rem; color:#94a3b8; font-size:0.8rem; text-transform:uppercase; letter-spacing:0.05em;">Referenced By (Incoming)</h4><div style="display:flex; flex-direction:column; gap:6px;">`; incoming.forEach(edge => {{ const src = nodeMap[edge.from]; if (src) {{ html += `<div style="background-color:rgba(255,255,255,0.03); border:1px solid #334155; padding:6px 10px; border-radius:6px; font-size:0.85rem; cursor:pointer;" onclick="selectNodeById('${{edge.from}}')"><strong style="color:${{src.color}};">${{src.label}}</strong> <span style="color:#94a3b8;">— ${{edge.rel}} →</span></div>`; }} }}); html += `</div></div>`; }}
        if (outgoing.length === 0 && incoming.length === 0) {{ html += `<p style="font-size:0.85rem; color:#94a3b8; font-style:italic;">No active relationships connected.</p>`; }}
    ...
    function searchNode() {{
        ...
        if (match) {{ selectNodeById(match.id); }} else {{ alert("Concept not found: " + query); }}
    }}
